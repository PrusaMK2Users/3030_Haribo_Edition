...
// Modified by Rob Baumstark <rbaumstark@gmail.com>
// for the 3030-Haribo Project
// https://github.com/PrusaMK2Users/3030_Haribo_Edition

// BPreplay font included is licensed under the SIL Open Font License.
// http://scripts.sil.org/OFL

// Instructions:

// If you want a number in the top-left corner, change the haribo_number value below to your number. If haribo_number is set to 0 then the corner is left blank.

// By default the text is engraved into the cover - if you need a second STL of the text to do a dual-color print, after exporting the cover to an STL, comment out the lcd_cover(); line below (put two /'s in front of it), and uncomment the colortext(); line, then compile and export again.

// --------------------------------------------------
// End-user configuration section - edit as required.
haribo_number = 0;
lcd_cover();
//colortext();
// --------------------------------------------------
...
// HARIBO Edition text
translate([-54,52,0.6]) rotate([180,0,0]) linear_extrude(height = th) 
{ text("HARIBO",font = "BPreplay:style=Bold", size=10, center=true); }

translate([-56.5,52,0.6]) rotate([180,0,270]) linear_extrude(height = th) 
{ text("3030",font = "BPreplay:style=Bold", size=5.5, center=true); }

translate([63,0,0]) {
translate([-67,52,0.6]) rotate([180,0,0]) linear_extrude(height = th) 
{ text("EDITION",font = "BPreplay:style=Bold", size=7, center=true); }

// Line over small text
translate( [ -66 , 41.5 , 0 ] )  cube( [ 32 , 1.6 , th ] );  
translate( [ -66 , 42.3 , 0 ] ) cylinder( h = th, r = 0.8, $fn=30);  
translate( [ -34 , 42.3 , 0 ] ) cylinder( h = th, r = 0.8, $fn=30);  
}

// HARIBO Number
if (haribo_number > 0) {
  translate([-73,5,0.6]) rotate([180,0,0]) linear_extrude(height = th) 
{ text(str("#",haribo_number),font = "BPreplay:style=Bold", size=6, center=true); }
}

// X sign on reset button
translate( [ 63 , 34 , 0 ] ) rotate([0,0,45]) cube( [ 2, 8, th ] );  
translate( [ 57.5 , 35.5 , 0 ] ) rotate([0,0,-45]) cube( [ 2, 8, th ] );  
}
